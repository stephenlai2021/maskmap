...
- 我們把地圖綁定到 id 為 map 的 dom 元素並指派給 `map` 這個變數, 這個動作只需要做一次, 這個專案我用 [quasar](https://quasar.dev/) 這個框架開發, 用以上的方式會報錯, 必須在 `L.map()` 指定 dom 元素, 要這樣 
```
const map = L.map(document.getElementById('map')).setView([lat, lng], zoom) /
